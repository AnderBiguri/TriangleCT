...
void graphProject(const unsigned long *elements, const double *vertices,const unsigned long *boundary,const long *neighbours, const float * d_image, float * d_res, Geometry geo,
...
//     mexPrintf("%u %.16f %.16f\n",(unsigned int)boundary[current_element]+1,t2,t1);
    ...
    while(!noNeighbours && c<100){
        ...
        t2=max4(t,&indM);
        t1=min4nz(t);
//         mexPrintf("%u %.16f %.16f\n",(unsigned int)current_element+1,t2,t1);
//         mexPrintf("%.16f \n",(t2-t1));
...
void graphForwardRay_CPU(float const * const  image,  Geometry geo,
    ...
    d_res=(float*)malloc(num_bytes_proj);
    for(int i = 0; i < geo.nDetecU*geo.nDetecV; i++){
        d_res[i] = 0.;
    }
    if (DEBUG_TIME){
...
//     vec3d ray1,ray2,triN1,triN2,triN3;
//     ray1.x=0.0000000000000612; ray1.y=1000.0000000000000000;  ray1.z=0.0000000000000000;
//     ray2.x=-98.5000000000000000; ray2.y=-536.0000000000000000; ray2.z=19.5000000000000000;
//
//   triN1.x=- 78; triN2.x=-78; triN3.x=-0.5;
//   triN1.y=6.4249997138977051; triN2.y=6.4249997138977051; triN3.y=5.3549995422363281;
//   triN1.z=-60;triN2.z= 60; triN3.z=-6;
//
//   moller_trumbore(ray1,ray2,triN1,triN2,triN3,0.0);
//     return;
...
//         unsigned int nitery=geo.nDetecV+inity; //geo.nDetecV;
        unsigned int initx=216-1;
        unsigned int inity=309-1;
        ...
        for(unsigned int x=initx;x<(unsigned int)niterx;x++){
            for(unsigned int y=inity;y<(unsigned int)nitery;y++){
                initXrays(elements,nodes,boundary,nboundary, d_res, geo, source2,deltaU, deltaV,uvOrigin,nodemin,nodemax,x,y);
                
                graphProject(elements,nodes,boundary,neighbours,image,d_res, geo,source2,deltaU,deltaV,uvOrigin,x,y);
                
            }
        }
//         mexPrintf("%f\n",d_res[226*geo.nDetecU+147]);
        memcpy(result[i], d_res, num_bytes_proj);
    }
