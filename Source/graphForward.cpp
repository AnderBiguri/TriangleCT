        ...
        int nrhs, mxArray const *prhs[])
{
     ...
     ********************************************************************/
    // Graph.
    mxArray * graphMex=(mxArray*)prhs[3];
    Graph graph = initGraph(graphMex);
     ...
     *********************** Run code ********************************
     ********************************************************************/
    graphForwardRay(img,geo,angles,nangles,graph,result);
    
    
    unfuckGraphTemp(&graph);
}
...
Graph initGraph(mxArray* graphMex){
    ...
    for (unsigned int i=0;i<node_size[1];i++){
        ...
        graph.node[i].nAdjacent= static_cast<unsigned int>(auxSize[1]);
    ...
    for (unsigned int i=0;i<element_size[1];i++){
        ...
        auxSize=mxGetDimensions(tmpNeigh);   // Should be 4 most of the times, unless boundary.
        
        
        graph.element[i].neighbour=(unsigned int*)malloc(auxSize[1]*sizeof(unsigned int));
        graph.element[i].neighbour=static_cast<unsigned int *>(mxGetData(tmpNeigh));
        graph.element[i].nNeighbour=static_cast<unsigned int>(auxSize[1]);
        for (unsigned int j=0;j< graph.element[i].nNeighbour;j++)
    ...
    graph.boundary=(unsigned int *)malloc(boundary_size[1] * sizeof(unsigned int));
    graph.boundary=static_cast<unsigned int *>(mxGetData(tmp));
